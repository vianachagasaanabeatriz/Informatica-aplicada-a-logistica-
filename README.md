## 📑 Índice

- [Atividade 1 – Quem é Quem?](#atividade-1--quem-é-quem)
- [Atividade 2 – Análise de Dados ANTT](#atividade-2--análise-de-dados-antt)
- [Atividade 3 – Operadores de Transporte Multimodal](#atividade-3--operadores-de-transporte-multimodal)
- [Atividade 4 – Análise de Fecundidade](#atividade-4--análise-de-fecundidade)
- [Atividade 5 – Dashboard de Empresas Multimodais](#atividade-5--dashboard-de-empresas-multimodais)
- [Atividade 6 – Dashboard de Fecundidade](#atividade-6--dashboard-de-fecundidade)

---
---

ATIVIDADE 6
# Atividade 6 – Dashboard de Fecundidade

### 📂 Arquivos da Atividade 06

➡️ [Acessar a pasta da Atividade 06](./Atividade-06/)

⬆️ [Voltar ao Índice](#-índice)

Nesta atividade foi desenvolvido um dashboard no **Microsoft Power BI** para análise dos dados de fecundidade do Estado de São Paulo.

O painel utiliza diferentes tipos de gráficos para facilitar a comparação dos dados ao longo dos anos, entre regiões e entre diferentes faixas etárias.

## 📊 Perguntas Analisadas

1. Como o número de nascidos variou ao longo dos anos no Estado de São Paulo?

2. Quais regiões apresentaram o maior número de nascidos?

3. Como a idade média das mães mudou ao longo dos anos?

4. Qual faixa etária apresentou maior fecundidade em determinado ano?

5. Quais regiões apresentaram a maior fecundidade total?

<img width="1761" height="922" alt="image" src="https://github.com/user-attachments/assets/25292dcf-4624-440e-956f-76126cbf621d" />


## 📈 Visualizações

Foram utilizados diferentes recursos visuais, como:

- Gráfico de linhas;
- Gráfico de barras;
- Gráfico de colunas;
- Treemap;
- Filtros por ano e região.

## 🛠️ Ferramentas Utilizadas

- Microsoft Power BI
- Power Query
- Tratamento de dados
- Visualização de dados

## 📁 Arquivo

`5 questões.pbix`

---


ATIVIDADE 5
# Atividade 5 – Dashboard de Empresas Multimodais

### 📂 Arquivos da Atividade 05

➡️ [Acessar a pasta da Atividade 05](./Atividade-05/)

⬆️ [Voltar ao Índice](#-índice)

## 📌 Sobre a Atividade

Nesta atividade foi desenvolvido um dashboard no **Microsoft Power BI** para análise de dados relacionados às empresas e operadores de transporte multimodal.

O objetivo foi transformar os dados disponíveis em informações visuais e interativas, facilitando a análise e comparação dos resultados.

## 📊 Dashboard

O projeto apresenta diferentes visualizações desenvolvidas no Power BI, permitindo analisar os dados de forma dinâmica por meio de gráficos, indicadores e filtros.

<img width="1674" height="940" alt="image" src="https://github.com/user-attachments/assets/10705962-c2af-48bf-9fd1-4763e1ef99f9" />


## 🛠️ Ferramentas Utilizadas

- Microsoft Power BI
- Power Query
- Visualização de Dados

## 📁 Arquivo do Projeto

`Empresasmultimodais.pbix`

O arquivo contém o dashboard desenvolvido no Microsoft Power BI.

---

ATIVIDADE 4

# Atividade 4 – Análise de Fecundidade

### 📂 Arquivos da Atividade 04

➡️ [Acessar a pasta da Atividade 04](./Atividade-04/)

⬆️ [Voltar ao Índice](#-índice)

## 📚 Fecundidade e Análise de Dados

A análise da fecundidade permite observar características relacionadas aos nascimentos e ao comportamento demográfico de uma população ao longo do tempo.

Nesta atividade, foi utilizada uma base de dados de fecundidade do Estado de São Paulo, contendo informações como ano, região, número de nascidos, fecundidade por faixa etária, fecundidade total e idade média das mães.

## 📊 Análises Realizadas

A partir da base de dados, foram elaboradas cinco perguntas:

1. Qual foi a variação no número de nascidos entre os anos de 2000 e 2024 nas regiões de São Paulo?
<img width="1006" height="604" alt="image" src="https://github.com/user-attachments/assets/08302943-1d9a-4bfa-9209-6ffb842965b8" />

2. Qual região apresentou a maior idade média das mães em 2024?
<img width="1094" height="697" alt="image" src="https://github.com/user-attachments/assets/3d2aa00d-ef35-461c-a54d-55550e7501fb" />


3. Como a idade média das mães evoluiu em São José dos Campos ao longo dos anos?

<img width="877" height="593" alt="image" src="https://github.com/user-attachments/assets/03152014-cced-4f7c-a77b-c54a6fd07506" />

4. Como a fecundidade se distribuiu entre as diferentes faixas etárias nas regiões de São Paulo em 2005?

<img width="1580" height="587" alt="image" src="https://github.com/user-attachments/assets/7d481491-f037-4078-a8c0-fffd3e690776" />

5. Como a fecundidade se distribuiu entre as diferentes faixas etárias nas regiões de São Paulo em 2020?
<img width="989" height="632" alt="image" src="https://github.com/user-attachments/assets/b068f472-13ba-4171-97fe-5277710ffd41" />

## 💻 Ferramentas utilizadas

- Microsoft Excel
- Fórmulas
- Tabelas
- Gráficos

## 📁 Base de Dados

A planilha contém informações sobre nascimentos, idade média das mães e fecundidade por faixa etária em diferentes regiões do Estado de São Paulo.


---


ATIVIDADE 3
# Atividade 3 – Operadores de Transporte Multimodal

### 📂 Arquivos da Atividade 03

➡️ [Acessar a pasta da Atividade 03](./Atividade-03/)

⬆️ [Voltar ao Índice](#-índice)

Nesta atividade foi utilizada uma base de dados sobre **Operadores de Transporte Multimodal**, com o objetivo de organizar, analisar e representar as informações por meio de tabelas e gráficos no Microsoft Excel.

## 📊 Perguntas analisadas

1. **Qual estado possui mais empresas autorizadas a realizar transporte multimodal?**

   <img width="949" height="611" alt="image" src="https://github.com/user-attachments/assets/039a555f-88aa-44fb-9af3-9b184b25de86" />


3. **Qual país de origem possui mais empresas autorizadas?**

   <img width="880" height="514" alt="image" src="https://github.com/user-attachments/assets/736e6b5e-45ac-4f7b-a57f-b88706903fd5" />


5. **Quantas empresas de cada UF aderiram ao Decreto 1563/95?**

   <img width="819" height="410" alt="image" src="https://github.com/user-attachments/assets/ae3872a4-b918-4830-867c-8650843ddfc1" />


## 📈 Desenvolvimento

Para responder às três perguntas, os dados foram organizados e analisados no Excel, utilizando tabelas dinâmicas e gráficos para facilitar a visualização e comparação dos resultados.

## 📁 Arquivo utilizado

`operador_transporte_multimodal.xlsx`

## 💻 Ferramenta utilizada

Microsoft Excel



---




ATIVIDADE 2

## Dados ANTT
# Atividade 2 – Análise de Dados ANTT   

### 📂 Arquivos da Atividade 02

➡️ [Acessar a pasta da Atividade 02](./Atividade-02/)

⬆️ [Voltar ao Índice](#-índice)

<img width="725" height="412" alt="image" src="https://github.com/user-attachments/assets/7168daf4-f4b3-498f-a429-8e739f27bce2" />

A partir da inclusão da Base de Dados foi possível elaborar uma tabela dinâmica específica para a consulta de CEP's por estados, posteriormente um gráfico dinâmico exemplifica e demonstra toda a resposta.


---




ATIVIDADE 1

# Atividade 1 – Quem é Quem?

### 📂 Arquivos da Atividade 01

➡️ [Acessar a pasta da Atividade 01](./Atividade-01/)

⬆️ [Voltar ao Índice](#-índice)

<img width="1419" height="798" alt="Captura de tela 2026-08-22 212103" src="https://github.com/user-attachments/assets/58e68ae6-13cf-499a-94ec-48b06e082f60" />
